...
if [ -n "${SGE_TASK_ID+x}" ]; then
    ...
    echo "What? ffmpeg -i $ORIGINAL/{VIDEO} -vcodec libx264 -preset medium -s 1280x720 -acodec copy $PROCESSED/{VIDEO}.mp4"
    echo "      for each {VIDEO} in file 'video_list'"
    echo "How? qsub -t 1-NSPLIT `basename $0` video_list"
fi
